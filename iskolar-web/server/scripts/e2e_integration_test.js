...
(async () => {
  ...
  assert(regRes.status === 200, 'Student registration succeeds', `got ${regRes.status}: ${JSON.stringify(regRes.body).slice(0, 200)}`);
  ...
  assert(provReg.status === 200, 'Provider registration succeeds', `got ${provReg.status}`);
  ...
  const appList = await GET('/api/applications', { Authorization: `Bearer ${seededStudentToken}` });
  ...
  console.log('\n── STEP 10: Application Status Update ──');
  const provApps = await GET('/api/applications', { Authorization: `Bearer ${providerToken}` });
  ...
  // 11b: List schedules
  const schedList = await GET('/api/schedules', { Authorization: `Bearer ${seededStudentToken}` });

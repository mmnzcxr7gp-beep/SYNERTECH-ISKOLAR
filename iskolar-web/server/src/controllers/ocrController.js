...
const fieldPatterns = {
  ...
  idNumber: [
    /(?:id\s*(?:no|number|#))\s*[:\-]?\s*([\w\-]+)/i,
    /(?:license\s*no|card\s*no)\s*[:\-]?\s*([\w\-]+)/i,
    /(?:CRN|TIN|SSS|GSIS|PhilHealth)\s*[:\-]?\s*([\d\-]+)/i,
  ],
  dateOfBirth: [
...
const extractFromDocument = async (req, res, next) => {
  try {
    ...
    return res.json({
      ...
      message: 'Document processed successfully. Please review and confirm the extracted information.',
      filePath: `/uploads/${path.basename(filePath)}`,
    });
  } catch (err) {
    next(err);

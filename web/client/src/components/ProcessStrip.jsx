...
export default function ProcessStrip() {
  return (
    <section className="py-6 px-4 md:px-8 max-w-7xl mx-auto" aria-label="Application Process Overview">
      <div
        className="modular-card p-3 sm:p-4 flex flex-wrap items-center justify-between gap-2 sm:gap-4"
      ...
      >
        ...
        <nav className="flex flex-wrap items-center gap-1 sm:gap-2 flex-1 justify-end" aria-label="Process Steps">
          {processSteps.map((step, idx) => (
            <React.Fragment key={step.id}>
              <a
                href={step.href}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition hover:bg-[var(--color-bg-elevated)]"
                style={{

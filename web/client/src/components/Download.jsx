...
const mobileHighlights = [
  ...
  {
    icon: <SearchIcon className="w-5 h-5 text-[#FF8552]" />,
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
          ...
          <div className="space-y-6">
            ...
            <div className="pt-2 flex flex-wrap gap-4">
              <ShinyButton size="lg" icon={<AndroidIcon className="w-4 h-4 text-white" />} onClick={() => alert('Download ISKOLAR Android APK package.')}>
                Download Android APK
              </ShinyButton>
              <ShinyButton variant="secondary" size="lg" icon={<AppleIcon className="w-4 h-4" />} onClick={() => alert('ISKOLAR iOS TestFlight client.')}>
                iOS TestFlight Beta
              </ShinyButton>
            </div>
          </div>
          ...
          <div>
            <SpotlightCard className="p-7 md:p-8 space-y-5">
              <div className="flex items-center justify-between pb-4 border-b" style={{ borderColor: 'var(--border)' }}>
                <span className="text-xs font-extrabold uppercase tracking-wider text-[#FF6D29]">
                ...
                {mobileHighlights.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      {item.icon}
                      <h4 className="text-sm font-extrabold" style={{ color: 'var(--text-heading)' }}>
                        {item.title}
                      </h4>
                    </div>

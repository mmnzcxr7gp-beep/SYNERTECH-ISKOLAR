...
const initTransporter = async () => {
  ...
  if (!emailUser) {
    console.warn('⚠️ SMTP Configuration Warning: EMAIL_USER environment variable is missing.');
  }
  if (!emailPass || emailPass.includes('<') || emailPass === 'your-app-password-here') {
    console.warn('⚠️ SMTP Configuration Warning: EMAIL_PASSWORD environment variable is missing or contains placeholder.');
  }

  if (emailUser && emailPass && !emailPass.includes('<')) {
    try {
      ...
      console.error('⚠️ Custom SMTP transport error:', err?.message || err);
    }
  }
...
/**
 * Send email to student when an exam or interview is scheduled
 */
const sendScheduleNotificationEmail = async ({
  studentEmail,
  studentName,
  type,
  title,
  ...
  const firstName = typeof studentName === 'string' ? studentName.split(' ')[0] : 'Student';
  const typeLabel = type === 'exam' ? 'Qualifying Examination' : 'Candidate Panel Interview';
  const subject = `Invitation: ${typeLabel} - "${title}"`;
  ...
  const html = `
    <div style="font-family: 'Plus Jakarta Sans', 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #0D1E3B; color: #F4F0E8; border-radius: 24px; border: 1px solid #C5A28C;">
      ...
      <h1 style="font-size: 22px; font-weight: 900; color: #FFFFFF; text-align: center; margin: 0 0 16px 0;">
        Hello, ${firstName}!
      </h1>

      <p style="font-size: 15px; line-height: 1.6; color: #C8D4E6; text-align: center; margin-bottom: 28px;">
        You have been scheduled for <strong>"${title}"</strong>. Please find your schedule details below:
      </p>

      <div style="background: #132644; border: 1px solid rgba(197, 162, 140, 0.3); border-radius: 16px; padding: 20px; margin-bottom: 28px;">
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
          <tr>
            <td style="padding: 8px 0; color: #8FA2C0;">Event:</td>
            <td style="padding: 8px 0; font-weight: 700; color: #FFFFFF; text-align: right;">${title}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding: 8px 0; font-weight: 700; color: #FFFFFF; text-align: right;">${time}${endTime ? ` - ${endTime}` : ''}</td>
          </tr>
          ${venue ? `<tr><td style="padding: 8px 0; color: #8FA2C0;">Venue / Room:</td><td style="padding: 8px 0; font-weight: 700; color: #F4F0E8; text-align: right;">${venue}</td></tr>` : ''}
          ${meetingLink ? `<tr><td style="padding: 8px 0; color: #8FA2C0;">Online Link:</td><td style="padding: 8px 0; font-weight: 700; color: #38BDF8; text-align: right;"><a href="${meetingLink}" style="color: #38BDF8;">${meetingLink}</a></td></tr>` : ''}
        </table>
      </div>

      ${notes ? `<p style="font-size: 13px; color: #C8D4E6; background: rgba(0,0,0,0.2); padding: 12px; border-radius: 8px; margin-bottom: 20px;"><strong>Notes:</strong> ${notes}</p>` : ''}

      <p style="font-size: 12px; color: #8FA2C0; text-align: center; margin: 0;">
        Please make sure to arrive on time or test your connection beforehand. Best of luck!
      </p>
    </div>
  `;

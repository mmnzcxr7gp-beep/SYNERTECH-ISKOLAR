...
async function runOcrAndSocketTests() {
  ...
  try {
    ...
    if (regA.status !== 200 && regA.status !== 201) {
      console.log('  ⚠️ regA debug:', regA.status, JSON.stringify(regA.body));
    }
    assert(regA.status === 200 || regA.status === 201, 'Student A registers successfully');
    const tokenA = regA.body.token;
    const userIdA = regA.body.user ? regA.body.user.id : regA.body.userId;
    ...
    if (ocrUpload.status !== 200) {
      console.log('  ⚠️ ocrUpload debug:', ocrUpload.status, JSON.stringify(ocrUpload.body));
    }
